...
export class ScraperService {
  ...
  async searchGoogle(product: Product): Promise<Product[]> {
    ...
    const noResults = this.util.getXPathString(doc, GoogleXPaths.g_step1_no_results_xpath);
    ...
    if (href.length === 0) {
      ...
      const prices = this.util.getXPathArray(doc, GoogleXPaths.g_step1_price_xpath);
      const arrPrices = [];
      ...
      const retailers = this.util.getXPathArray(doc, GoogleXPaths.g_step1_retailer_xpath);
      const arrRetailers = [];
      ...
      const titles = this.util.getXPathArray(doc, GoogleXPaths.g_step1_title_xpath);
      const arrTitles = [];
  ...
  private async getGooglePrices(id: string, search: string) {
    ...
    const prices = this.util.getXPathArray(doc, GoogleXPaths.g_step2_price_xpath);
    node = prices.iterateNext();
    ...
    const title = this.util.getXPathString(doc, GoogleXPaths.g_step2_title_xpath);
    const data = [];
