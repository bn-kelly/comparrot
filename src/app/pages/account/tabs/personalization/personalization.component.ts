...
export class PersonalizationComponent implements OnInit {
  ...
  async ngOnInit() {
    const { categories, types } = await this.firebaseService.docAsPromise(`preference/size`);
    this.categories = categories;
    this.types = types;

    this.sizes = await this.firebaseService.collectionWithCache(`product_size`)
  ...
  selectSizeValue(sid: string, type: string, value: string, emitChange = false) {
      ...
      const sizes = this.sizes
        ...
        .map((size) => {
          ...
          size.value.forEach((sv: any) => {
            const type = Object.keys(sv)[0];
            const values = sv[type].filter((v) => !!this.sizesMap.get(`${size.id}${type}${v}`));
